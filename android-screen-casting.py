...
class Scrcpy:
	...
	def connect(self,device):
		...
		if (device.video==False): scrcpy_command += " --no-video"
		if (device.audio==False): scrcpy_command += " --no-audio"
		if (device.borderless==False): scrcpy_command += " --no-audio"
		if (device.fullscreen==True): scrcpy_command += " --fullscreen"
...
class Device:
	def __init__(self, **kwargs):
		self.__dict__.update(kwargs)
		self.id	= kwargs.get('serial', None) # to specify devices
